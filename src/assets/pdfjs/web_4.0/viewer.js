...
/**
 * Helper function to parse query string for Viewer.
 */
function parseDocumentPath(query) {
    var path = query.split('file=');
    var params = {};
    var pathParts = (path[1].indexOf('?') > 0) ? path[1].split('?') : path[1].split('&');

    var queryString = (pathParts.length == 2) ? pathParts[1].split('&') : path[1].split('&');
    ...
    for (var i = 0; i < queryString.length; i++) {
        ...
        // encode for object_id contains hash sign(#)
        value = (decodeURIComponent(key) == 'r_object_id' && decodeURIComponent(value).indexOf('#') > -1) ? encodeURIComponent(value) : decodeURIComponent(value)
        params[decodeURIComponent(key)] = value;
        urlVars = urlVars.concat('&', decodeURIComponent(param[0]), '=', value);
    }

    params['file'] = decodeURIComponent(pathParts[0]).concat('?', urlVars.charAt(0) === '&' ? urlVars.slice(1) : urlVars);
...
var PDFLinkService = (function () {
    ...
    PDFLinkService.prototype = {
        ...
        /**
         * @param dest - The PDF destination object.
         * @returns {string} paramaters (page and zoom) to the PDF object.
         */
        getDestinationHashWithoutBaseUrl: function PDFLinkService_getDestinationHashWithoutBaseUrl(dest) {
            var result = '';
            if (typeof dest === 'string') {
                result = '#' + escape(dest);
            }
        ...
        * @param {String} anchor The anchor hash, including the #
        */
        resolveUrl: function PDFLinkService_resolveUrl(baseUrl, link) {
            var parts = link.replace(/\\/g, '/').split('/');
            var parts1 = baseUrl.split('/');
            var count = 0;
            ...
            if (parts1.length > count) {
                var result = [];
                for (var i = 0; i < parts1.length - count - 1; i++) {
                    result[i] = parts1[i];
                }
...
var PDFOutlineViewer = (function PDFOutlineViewerClosure() {
    ...
    PDFOutlineViewer.prototype = {
        ...
        /**
        * @private
        * _bindOutlineExternalLink
        */

        _bindOutlineExternalLink: function PDFOutlineViewer_bindOutlineExternalLink(element, item) {
            var linkService = this.linkService;

            if (item.url) {
                //add parameters (zoom + page) from dest.
                var dest = linkService.getDestinationHashWithoutBaseUrl(item.dest);
                element.href = element.title = item.url + dest;
                element.rel = "external-link";
                element.onclick = function () {
                    var parts = item.url.replace(/\\/g, '/').split('/');
                    var relativePath = "";

                    if (!PDFJS.isValidUrl(element.title, false)) {
                        var baseUrl = linkService.baseUrl.split('?');
                        var currentUrl = "DocView.aspx?url=" + baseUrl[0];
                        var tabName = parts[parts.length - 1];
                        var extension = parts[parts.length - 1].split('.')[1];
                        relativePath = linkService.resolveUrl(baseUrl[0], element.title);
                    } else {
                        relativePath = element.title;
                    }

                    if (window.top && window.top.OpenDocumentFromDocInfo) {
                        element.href = "";
                        if (tabName != "" && typeof (tabName) != "undefined") {
                            window.top.OpenDocumentFromDocInfo(relativePath, currentUrl, tabName, extension);
                        } else {
        ...
        render: function PDFOutlineViewer_render(params) {
            ...
            while (queue.length > 0) {
                ...
                for (var i = 0, len = levelData.items.length; i < len; i++) {
                    ...
                    if (item.url) {
                        this._bindOutlineExternalLink(element, item);
                    } else {
...
var PDFViewerApplication = {
    ...
    load: function pdfViewLoad(pdfDocument, scale) {
        ...
        // VWR: Custome code for Viewer
        //var baseDocumentUrl = null;
        var baseDocumentUrl = this.url;
...
function webViewerInitialized() {
    ...
    var params = parseDocumentPath(queryString);
